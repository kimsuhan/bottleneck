...
Group = (function() {
  class Group {
    ...
    async clusterKeys() {
      var cursor, error, found, i, k, keys, len, logicalId, next, prefix;
      if (this.connection == null) {
      ...
      cursor = null;
      while (cursor !== 0) {
        [next, found] = (await this.connection.__runCommand__(["scan", cursor != null ? cursor : 0, "match", Scripts.settingsPattern(), "count", 10000]));
        cursor = ~~next;
